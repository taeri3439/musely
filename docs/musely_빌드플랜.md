...
- 성미: REST/CRUD 기본기, 외부 AI 서비스 연동, 폴링 클라이언트 구현, 타임아웃·에러 처리
...
**왜 MQ가 아닌가** — MQ+콜백은 처리 시간이 길고(수십 초~분) 신뢰성이 중요한 워크로드에 맞는 패턴이다. 이번 규모(요청량 적음, 처리 5~10초)에선 브로커 운영 비용이 얻는 것보다 크고, 성미의 CRUD 학습 시간을 잠식한다.
...
**1주차에 mock 서버가 안 나오면 전체 일정이 밀린다.** 데이터 수집이 지루해서 미루게 되는데, mock만 먼저 띄워두면 성미가 3주차까지 안 막힌다.
